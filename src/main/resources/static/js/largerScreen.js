	...
	if (typeof(params.data) === "undefined") {
		...
		var wait = setInterval(function() {
			if (!$('.topHideenArea').is(":animated")) {
				...
				$('.leftHideenArea').hide();
				$('.videoArea').addClass('animated slideInDown');
				$('.videoArea').show();
				$(".mainTitle").hide();
				$(".videoTitle").show();
				$("#videOnwer").html(params.data.name);
				$('#bigVideo')[0].play();
...
function hiddenVideoPlayer() {
	$('.videoArea').removeClass('animated slideInDown');
	$('.videoArea').addClass('animated slideOutDown');
	$('.topHideenArea').show();
	...
	var wait = setInterval(function() {
		if (!$('.topHideenArea').is(":animated")) {
			...
			//执行code
			$('.videoArea').hide();
			$('.videoArea').removeClass('animated slideOutDown');
		}
	}, 600);
}
...
function stuffTeachingPlaceTable(tableInfo) {
	window.teachingPlaceEvents = {
		'click #moreInfo': function(e, value, row, index) {
			changeToShowLocalInfo(row, index);
		},
		'click #samllVidebackConfig': function(e, value, row, index) {
			samllVidebackConfig(row, index);
		}
	};
...
function changeToShowLocalInfo(row, index) {
	var currentIsPlaySmallVideo;
	$(".smallVideoArea").is(':hidden') ? currentIsPlaySmallVideo = false : currentIsPlaySmallVideo = true;
	if (currentIsPlaySmallVideo) {
		return
	}

	$('#map').addClass('animated bounceOut');
	var wait = setInterval(function() {
		if (!$('#map').is(":animated")) {
			clearInterval(wait);
			//执行code
			$('#map').removeClass("animated bounceOut").hide();
			$('.smallVideoArea').show().addClass("animated bounceIn");
			$(".smallvideoTitle").show();
			$("#smallvideOnwer").html(row.studyLocalName);
			$("#smallVideo").attr("src", "testVideo/介绍.mp4");
			$(".hiddenBtn" + index).show();
			$(".moreInfoBtn").hide();
			$('#smallVideo')[0].play();
			...
			var attendanceInfo = [{
					name: '**市',
					type: 'line',
					data: [40, 5, 35, 80, 40, 60, 70]
				},
				{
					name: '**市',
					type: 'line',
					data: [30, 90, 47, 25, 58, 96, 97]
				},
				{
					name: '**市',
					type: 'line',
					data: [40, 38, 56, 69, 58, 58, 56]
				},
				{
					name: '**市',
					type: 'line',
					data: [85, 58, 68, 87, 25, 58, 55]
				},
				{
					name: '**市',
					type: 'line',
					data: [90, 88, 45, 46, 58, 85, 100]
				}
			];
			drawAttendanceCharts("attendance", attendanceInfo, row.studyLocalName);

			var maleToFemaleRatioInfo = {
				city: ['**市', '**市', '**市', '**市', '**市'],
				woman: [20, 50, 40, 35, 69],
				man: [80, 50, 60, 65, 31]
			}
			maleToFemaleRatioCharts("sexPart", maleToFemaleRatioInfo, row.studyLocalName);
			...
			occupationalDistributionCharts("studentJob", occupationalDistributionInfo, row.studyLocalName);

			var classHourInfo = [150, 351, 260, 290, 198];
			classHourCharts("classHours", classHourInfo, row.studyLocalName);

			var avgMonthlyIncomeInfo = [5981, 7014, 3399, 6558, 4111];
			avgMonthlyIncome("income", avgMonthlyIncomeInfo, row.studyLocalName);
			...
			ageDistribution("agePart", ageDistributionInfo, row.studyLocalName);
...
function samllVidebackConfig(row, index) {
	$('.smallVideoArea').hide();
	$('#map').addClass('animated bounceIn').show();
	$(".hiddenBtn" + index).hide();
	$(".moreInfoBtn").show();
	$('#smallVideo')[0].pause();
	//重新渲染六个小chart
	//演示版
