...
//字符类型 金额格式化
function undoNubmerMoneyMatter(value, row, index) {
    return [ '<div class="myTooltip" title="￥' + undoNubmer(value) + '">￥' + undoNubmer(value) + '</div>' ]
        .join('');
}

//数字类型 金额格式化
function dealNumberMoneyMatter(value, row, index) {
    return [ '<div class="myTooltip" title="￥' + dealNumber(value) + '">￥' + dealNumber(value) + '</div>' ]
        .join('');
}
