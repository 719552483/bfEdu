...
public interface Edu300Dao extends JpaRepository<Edu300, Long>, JpaSpecificationExecutor<Edu300> {
	...
	@Query(value = "select * FROM(SELECT a.*,row_number ( ) over ( partition BY njbm,njmc,batch,batch_name ORDER BY njbm,njmc,batch,batch_name ) su FROM edu300 a) e where e.su = 1",nativeQuery = true)
