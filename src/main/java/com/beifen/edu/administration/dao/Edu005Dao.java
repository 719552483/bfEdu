...
public interface Edu005Dao extends JpaRepository<Edu005, Long>, JpaSpecificationExecutor<Edu005> {
    ...
    //根据课程名称和生源类型查询成绩
    @Query(value = "select e.* from edu005 e LEFT JOIN edu001 ee on e.EDU001_ID = ee.EDU001_ID where ee.sylxbm = ?2 and COURSE_NAME in ?1 and e.xnid = ?3", nativeQuery = true)
    List<Edu005> updateMXStatusByCourse(List<String> courserName,String sylxbm,String trem);
