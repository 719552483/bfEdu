...
public interface Edu301Dao extends JpaRepository<Edu301, Long>, JpaSpecificationExecutor<Edu301> {
	...
	//任务书查找教学班
	@Query(value = "select distinct e.* from edu301 e,edu302 f,edu300 d where e.EDU301_ID = f.EDU301_ID and f.EDU300_ID = d.EDU300_ID and d.SFSCKKJH = 'T' and d.XBBM in ?1",nativeQuery = true)
	List<Edu301> findTeachingClassForTask(List<String> departments);

	//根据权限查找行政班
	@Query(value = "select * from edu301 where user_key = ?1",nativeQuery = true)
	List<Edu301> findAllInDepartment(String userKey);

	//根据权限查找行政班
	@Query(value = "select * from edu301 where user_key is null",nativeQuery = true)
	List<Edu301> findAllInDepartment2();

	//根据权限查找行政班
	@Query(value = "select * from edu301 where user_key is null and jxbmc like %?1%",nativeQuery = true)
	List<Edu301> findAllInDepartment2(String teachingClassName);
