...
//			}
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> verifiyImportEmploymentStudentsFile(MultipartFile file,String checkType,String hopeSheetName) throws java.text.ParseException, Exception {
		Map<String, Object> returnMap = new HashMap();
		boolean isExcel=false;
		boolean sheetCountPass=false;
		boolean modalPass=false;
		...
		if(isExcel){
			...
			//验证sheet名字
			String sheetName = workBook.getSheetAt(0).getSheetName();//sheet名称
			if (sheetCountPass&&checkType.equals("ImportEdu0011")&&sheetName.equals(hopeSheetName)) {
				modalPass=true;
			}
			...
			//验证数据正确性
			if(sheetCountPass&&modalPass&&haveData){
				if(checkType.equals("ImportEdu0011")){
					Map<String, Object> datacheckInfo=this.checkEmploymentStudents(importStudents);
					dataCheck=datacheckInfo.get("chaeckPass");
					checkTxt=datacheckInfo.get("checkTxt");
					importStudentInfo=datacheckInfo.get("importStudentInfo");
				}
			}
		}
		returnMap.put("isExcel",isExcel);
		...
		returnMap.put("dataCheck",dataCheck );
		returnMap.put("checkTxt", checkTxt);
		returnMap.put("importStudentInfo",importStudentInfo);
		return returnMap;
	}
	 ...
	 * @throws java.text.ParseException
	 * */
	private Map<String, Object> checkEmploymentStudents(List<Map<String, Object>> importTeachers) throws ParseException, Exception {
		Map<String, Object> returnMap = new HashMap();
		...
		returnMap.put("chaeckPass", chaeckPass);
		returnMap.put("checkTxt", checkTxt);
		returnMap.put("importStudentInfo", importTeacher);
		return returnMap;
	}
...
//			}
				...
				if (!isRowEmpty(row)) {
					...
					for (int cIndex = firstCellIndex; cIndex < lastCellIndex; cIndex++) { // 遍历列
						...
						if (cell != null && !cell.equals("")) {
							...
							if(keyType.equals("ImportEdu0011")){
								keyName = getImportantEdu0011KeyName(cell.getColumnIndex()); //获取列名
							}
	...
	//获取导入学生Excel的Key值
	private String getImportantEdu0011KeyName(int columnIndex) {
		String result = null;
		switch (columnIndex) {
			case 0:
				result="num";
...
//			appendCell(sheet,i,"",chosedStudents.get(i).getSfzh(),-1,12,false);
    ...
    private void stuffTeacherInfoSheet1(XSSFSheet sheet,List<Edu101> chosedTeachers){
		...
		XSSFCell cells[] = new XSSFCell[1];
		// 所有标题数组
		String[] titles = new String[] {"*教职工ID","*姓名","*性别", "*教职工类型", "*出生日期", "身份证号", "二级学院","所属专业",
	...
	// 下载模板2
	public void loadModal2(HttpServletResponse response,String filename, XSSFWorkbook workbook) throws IOException, ParseException {
		setEXCELstyle2(workbook);
		setEXCELformatter(workbook);
		ExcelStuffSelect(workbook,filename);
		// 解决导出文件名中文乱码
		response.setCharacterEncoding("UTF-8");
	...
	public void loadModalwithNote(HttpServletResponse response,String filename, XSSFWorkbook workbook) throws IOException, ParseException {
		setEXCELstyleWithNote(workbook);
		setEXCELformatter(workbook);
		ExcelStuffSelect(workbook,filename);
		// 解决导出文件名中文乱码
		response.setCharacterEncoding("UTF-8");
...
//			}
	...
	//设置模title样式2
	private void setEXCELstyle2(XSSFWorkbook workbook) throws ParseException {
		workbook.getSheetAt(0).getRow(0).setHeightInPoints(30); //sheet1标题行行高设置

		for(int i = 0;i<=2;i++){
			Row row = workbook.getSheetAt(0).getRow(i); //  sheet1第一行标题行
			//sheet1 样式
			int firstCellIndex = row.getFirstCellNum(); //  sheet1第一行标题行第一cell
			int lastCellIndex = row.getLastCellNum(); //  sheet1第一行标题行最后cell
			//遍历sheet1 标题行cell
	...
	//设置模title样式
	private void setEXCELstyleWithNote(XSSFWorkbook workbook) throws ParseException {
		workbook.getSheetAt(0).getRow(1).setHeightInPoints(30); //sheet1标题行行高设置

		//sheet1 样式
		Row row = workbook.getSheetAt(0).getRow(1); //  sheet1第一行标题行
		int firstCellIndex = row.getFirstCellNum(); //  sheet1第一行标题行第一cell
		int lastCellIndex = row.getLastCellNum(); //  sheet1第一行标题行最后cell
		//遍历sheet1 标题行cell
		for (int cIndex = firstCellIndex; cIndex <= lastCellIndex; cIndex++) {
...
//	}
	 ...
	 * @throws InvocationTargetException
	 */
	public void copyParm(Object source, Object target) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
	...
	public String listToString(List list, char separator) {
		String join = "";
		if (list.size() != 0) {
			join = StringUtils.join(list.toArray(), separator);
		}
		return join;
	}
	...
	public static List<List<Object[]>> splitList2(List<Object[]> messagesList, int groupSize) {
		int length = messagesList.size();
		// 计算可以分成多少组
		int num = (length + groupSize - 1) / groupSize; // TODO
